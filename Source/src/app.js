; (function () {
    ...
    async function toggleAiVoiceAgent() {
        ...
        try {
            ...
            const provider = await window.mauzer.ai.getProvider();
    ...
    async function connectOpenAIVoice() {
        ...
        const tokenResult = await window.mauzer.ai.getRealtimeToken();
...
6. НИКОГДА не говори "я использую инструмент" — ты ЧЕЛОВЕК`,
            ...
            try {
                ...
                if (msg.type === 'response.function_call_arguments.done') {
                    ...
                    if (toolName === 'open_website') {
                        ...
                        await window.mauzer.ai.executeAction({ type: 'navigate', url: toolArgs.url });
                    }
                        ...
                        await window.mauzer.ai.executeAction({ type: 'navigate', url: url });
                    }
                        ...
                        const jsCode = `
                            ...
                            `;
                        const r = await window.mauzer.ai.executeAction({ type: 'executeJS', code: jsCode });
                        ...
                        await window.mauzer.ai.executeAction({ type: 'executeJS', code: `window.scrollBy({top:${px},behavior:'smooth'})` });
                    }
                    else if (toolName === 'go_back') {
                        dom.aiOrbStatus.textContent = "Назад...";
                        await window.mauzer.ai.executeAction({ type: 'executeJS', code: `history.back()` });
                    }
                    else if (toolName === 'go_forward') {
                        dom.aiOrbStatus.textContent = "Вперёд...";
                        await window.mauzer.ai.executeAction({ type: 'executeJS', code: `history.forward()` });
                    }
                    else if (toolName === 'describe_page') {
                        dom.aiOrbStatus.textContent = "Смотрю...";
                        try {
                            const screenshot = await window.mauzer.ai.captureTab();
                            if (screenshot && screenshot.base64) {
                                // Send screenshot to GPT-4o REST API for actual vision analysis
                                const visionResult = await window.mauzer.ai.describeScreen(screenshot.base64);
        ...
        const sdpResult = await window.mauzer.ai.sendSDP(EPHEMERAL_KEY, offer.sdp);
        if (sdpResult.error) {
    ...
    async function connectGeminiVoice() {
        ...
        const result = await window.mauzer.ai.connectGemini();
        if (result.error) {
        ...
        processor.onaudioprocess = (e) => {
            ...
            window.mauzer.ai.sendGeminiAudio(base64);
        };
        ...
        window.mauzer.ai.onGeminiEvent(async (msg) => {
            if (!aiState.isActive) return;
            ...
            if (msg.toolCall && msg.toolCall.functionCalls) {
                for (const fc of msg.toolCall.functionCalls) {
                    ...
                    if (toolName === 'open_website') {
                        ...
                        await window.mauzer.ai.executeAction({ type: 'navigate', url: `https://www.google.com/search?q=${encodeURIComponent(toolArgs.query)}` });
                    } else if (toolName === 'search_youtube') {
                        dom.aiOrbStatus.textContent = "YouTube...";
                        await window.mauzer.ai.executeAction({ type: 'navigate', url: `https://www.youtube.com/results?search_query=${encodeURIComponent(toolArgs.query)}` });
                    } else if (toolName === 'click_text') {
                        dom.aiOrbStatus.textContent = "Кликаю...";
                        await window.mauzer.ai.executeAction({ type: 'executeJS', code: `(function(){const e=document.evaluate("//*[contains(text(),'${toolArgs.text.replace(/'/g, "\\'")}')]",document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null);if(e.singleNodeValue)e.singleNodeValue.click();})()` });
                    } else if (toolName === 'type_text') {
                        dom.aiOrbStatus.textContent = "Печатаю...";
                        await window.mauzer.ai.executeAction({ type: 'executeJS', code: `(function(){const i=document.querySelector('input[placeholder*="${toolArgs.target_text}"],textarea[placeholder*="${toolArgs.target_text}"]');if(i){i.focus();i.value='${toolArgs.value.replace(/'/g, "\\'")}';i.dispatchEvent(new Event('input',{bubbles:true}))}})()` });
                    } else if (toolName === 'scroll') {
                        dom.aiOrbStatus.textContent = "Скролл...";
                        await window.mauzer.ai.executeAction({ type: 'executeJS', code: `window.scrollBy(0, ${toolArgs.direction === 'down' ? 500 : -500})` });
                    } else if (toolName === 'go_back') {
                        await window.mauzer.ai.executeAction({ type: 'executeJS', code: `history.back()` });
                    } else if (toolName === 'go_forward') {
                        await window.mauzer.ai.executeAction({ type: 'executeJS', code: `history.forward()` });
                        ...
                        try {
                            ...
                            if (screenshot && screenshot.base64) {
                                ...
                                await window.mauzer.ai.sendGeminiImage(screenshot.base64);
                    ...
                    // Send tool response
                    await window.mauzer.ai.sendGeminiToolResponse([{
    ...
    function closeAiVoiceAgent() {
        ...
        if (aiState._provider === 'gemini') {
            window.mauzer.ai.disconnectGemini();
            window.mauzer.ai.removeGeminiListeners();
        }
